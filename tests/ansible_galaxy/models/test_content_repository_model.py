...
def test_frozen(csf):
    cr = ContentRepository(content_spec=csf)
...
def test_cr_init(csf):
    ...
    assert isinstance(cr, ContentRepository)
...
def CR(content_spec=None, path=None):
    if path:
        return ContentRepository(content_spec=content_spec, path=path)
    return ContentRepository(content_spec=content_spec)
